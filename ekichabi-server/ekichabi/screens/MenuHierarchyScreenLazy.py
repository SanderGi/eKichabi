...
from ekichabi.screens.base_screens.LazyScreenWrapper import LazyScreenWrapper
...
class MenuHierarchyScreenLazy(LongMenuScreen):
        ...
        if len(filters) > 0:
            ...
            for item in filter_vals:
                ...
                if new_query.count() > 1:
                    screen = MenuHierarchyScreenLazy(
                        self.next_filters, new_query.all(), new_selected)
                else:
                    screen = LazyScreenWrapper(lambda query=new_query[0]: BusinessDetailsScreen(query, False))
            ...
            if len(query_set) < 100 and len(items) > 1:
                ...
                items = [(see_all, MenuHierarchyScreenLazy([], query_set))] + items
            ...
            items = [(b.name, LazyScreenWrapper(lambda b=b, i=i, count=len(query_set): BusinessDetailsScreen(b, i < count - 1))) for i, b in enumerate(query_set)]
